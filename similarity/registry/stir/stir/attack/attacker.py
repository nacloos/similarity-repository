...
class Attacker(ch.nn.Module):
    ...
    def forward(self, x, target, *_, constraint, eps, step_size, iterations,
                model2=None, model2_target=None, random_start=False, random_restarts=False, do_tqdm=False,
                targeted=False, custom_loss=None, should_normalize=True,
                orig_input=None, use_best=True, return_image=True,
                est_grad=None, mixed_precision=False, devices=[0], 
                layer_num=None):
        '''
        ...
        Args:
            ...
            model2 (nn.Module): the second model, to be used for CKA/CCA
        ...
        orig_input = orig_input.to(x.device)
        ...
        def calc_loss(inp, target):
            ...
            if custom_loss:
                return custom_loss(self.model, model2, inp, target, model2_target, layer_num=layer_num)
        ...
        def get_adv_examples(x):
            ...
            def replace_best(loss, bloss, x, bx):
                if bloss is None:
                    ...
                    if len(replace.shape) > 0:
                        bx[replace] = x[replace].clone().detach()
                        bloss[replace] = loss[replace]
                    else:
                        # the case where we have a scalar loss, eg, when attacking CKA/CCA
                        bx = x.clone().detach()
                        bloss = loss
            ...
            for _ in iterator:
                ...
                assert len(losses.shape) == 0 or losses.shape[0] == x.shape[0], \
                ...
                with ch.no_grad():
                    ...
                    if do_tqdm:
                        iterator.set_description(
                            "Current loss: {l}, {desc}".format(
                                l=loss, desc=str(custom_loss)))

            # Save computation (don't compute last loss) if not use_best
            if not use_best: 
                ret = x.clone().detach()
                return step.to_image(ret) if return_image else \
                    ret, ch.tensor(all_losses).to(f'cuda:{devices[0]}')

            losses, _ = calc_loss(step.to_image(x), target)
            args = [losses, best_loss, x, best_x]
            best_loss, best_x = replace_best(*args)
            return step.to_image(best_x) if return_image else \
                best_x, ch.tensor(all_losses).to(f'cuda:{devices[0]}')
        ...
        if random_restarts:
            ...
            for _ in range(random_restarts):
                ...
                # use without random restarts for now
                new_losses, _ = calc_loss(adv, target)
                new_losses = ch.mean(new_losses).item()
                misclass = m*new_losses < m*losses
...
class AttackerModel(ch.nn.Module):
    ...
    def forward(self, inp, make_adv=False, with_latent=False, return_possible_layers=False,
        ...
        """
        if make_adv:
            # assert target is not None or not attacker_kwargs['targeted']
            prev_training = bool(self.training)
            self.eval()
            adv, losses = self.attacker(inp, **attacker_kwargs)
            if prev_training:
                self.train()
            inp = adv
        ...
        if 'layer_num' not in attacker_kwargs:
            layer_num = None
        else:
            layer_num = attacker_kwargs['layer_num']

        output = self.model(normalized_inp, with_latent=with_latent, 
                            return_possible_layers=return_possible_layers,
                            fake_relu=fake_relu, no_relu=no_relu, layer_num=layer_num)
        output_tup = (output, inp) if with_image else output
        return (output_tup, losses) if make_adv else output_tup
