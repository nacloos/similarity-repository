...
from mouse_vision.core.default_dirs import INTERAN_IM_SS_DIR
...
def build_param_lookup(source_map_kwargs=None,
                       ...
                       shorten_name=False,
                       report_train=False,
                       n_ss_imgs_arr=list(np.arange(20, NUM_NS_STIM, 20)) + [None],
                       n_jobs=5):
    ...
    param_lookup = {}
    key = 0
    for n_ss_imgs in n_ss_imgs_arr:
...
def compute_interanimal_consistencies(source_N,
    ...
    if n_ss_imgs is not None:
        ...
        num_total_images = target_N.shape[1]
        assert(n_ss_imgs < target_N.shape[1])
        assert(n_ss_imgs < source_N.shape[1])
    ...
    for curr_sp in train_test_splits:
        input_dict = {'target_N': target_N, 'source_N': source_N}
        if n_ss_imgs is not None:
            ss_im_idx = np.random.RandomState(seed=n_ss_im_seed).permutation(num_total_images)[:n_ss_imgs]
            input_dict = {'target_N': target_N[:, ss_im_idx, :], 'source_N': source_N[:, ss_im_idx, :]}
            # different set of images on each split
            n_ss_im_seed += 1
    ...
    filename = os.path.join(INTERAN_IM_SS_DIR, filename)
    if report_train:
