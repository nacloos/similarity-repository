...
def gram_rbf(x, device, threshold=1.0):
...
def center_gram(gram, unbiased=False):
  ...
  if unbiased:
    ...
    gram[range(n), range(n)] *= 0
    means = torch.sum(gram, dim=0, keepdim=True) / (n - 2)
...
def cka(gram_x, gram_y, debiased=False):
  ...
  scaled_hsic = torch.sum(gram_y * gram_x)

  normalization_x = torch.norm(gram_x, p='fro')
  normalization_y = torch.norm(gram_y, p='fro')
...
def feature_space_linear_cka_pytorch(features_x, features_y, debiased=False):
